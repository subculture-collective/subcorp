...
async function fetchOgImages(
    ...
    const queue = articles
        ...
        .filter(
            (item): item is { url: string; index: number } => item.url !== null,
        );
...
function curateArticles(
    ...
    const scored = items.map(item => ({
        ...
        source: item.source,
        score:
            (PRIORITY_CATEGORIES[item.category] ?? 0) +
            (item.index < 10 ? 2
            : item.index < 20 ? 1
            : 0),
    }));
...
export async function generateDailyNewspaper(): Promise<string | null> {
    ...
    if (recentItems.length < 5) {
        log.debug('Not enough RSS items for newspaper', {
            count: recentItems.length,
        });
        return null;
    }

    log.info('Generating newspaper edition', {
        date: today,
        candidates: recentItems.length,
    });
    ...
    postToDiscord(headline, articles.length, today, markdown).catch(err =>
        log.warn('Discord newspaper post failed', {
            error: (err as Error).message,
        }),
    );
...
export async function getEdition(
    date: string,
): Promise<(Omit<NewspaperEdition, 'pdf_data'> & { has_pdf: boolean }) | null> {
    const rows = await sql<
        (Omit<NewspaperEdition, 'pdf_data'> & { has_pdf: boolean })[]
    >`
        SELECT
